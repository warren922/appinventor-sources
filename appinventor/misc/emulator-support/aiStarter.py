...
def replstart(device=None):
    ...
    subprocess.check_output((PLATDIR + "commands-for-Appinventor/adb -s %s shell am start -a android.intent.action.VIEW -n life.youcodia.companion/.Screen1 --ez rundirect true") % device, shell=True, close_fds=True)
    response.headers['Access-Control-Allow-Origin'] = '*'
    response.headers['Access-Control-Allow-Headers'] = 'origin, content-type'
    return ''
